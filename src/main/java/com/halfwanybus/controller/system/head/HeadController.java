package com.halfwanybus.controller.system.head;
...
import com.halfwanybus.service.system.fhsms.FhsmsManager;
import com.halfwanybus.service.system.user.UserManager;
import com.halfwanybus.util.AppUtil;
import com.halfwanybus.util.Const;
import com.halfwanybus.util.Jurisdiction;
import com.halfwanybus.util.PageData;
import com.halfwanybus.util.SmsUtil;
import com.halfwanybus.util.Tools;
import com.halfwanybus.util.Watermark;
import com.halfwanybus.util.mail.SimpleMailSender;
...
//			String strWEBSOCKET = Tools.readTxtFile(Const.WEBSOCKET);//读取WEBSOCKET配置
		/*	if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
				String strIW[] = strWEBSOCKET.split(",halfwanybus,");
				if(strIW.length == 5){
...
//		String strWEBSOCKET = Tools.readTxtFile(Const.WEBSOCKET);//读取WEBSOCKET配置
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",halfwanybus,"+pd.getString("PORT")+",halfwanybus,"+pd.getString("EMAIL")+",halfwanybus,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",halfwanybus,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",halfwanybus,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",halfwanybus,"+pd.getString("fcontent")+",halfwanybus,"+pd.getString("fontSize")+",halfwanybus,"+pd.getString("fontX")+",halfwanybus,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",halfwanybus,"+pd.getString("imgUrl")+",halfwanybus,"+pd.getString("imgX")+",halfwanybus,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
//		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",halfwanybus,"+pd.getString("WIMPORT")+",halfwanybus,"+pd.getString("OLIP")+",halfwanybus,"+pd.getString("OLPORT")+",halfwanybus,"+pd.getString("FHsmsSound"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
