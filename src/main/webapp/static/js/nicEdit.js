...
var bkElement = bkClass.extend({
	...
	pos : function() {
		...
		var A = (!window.opera) ? parseInt(this.getStyle("border-width")
				|| this.style.border) || 0 : 0;
	...
	hasClass : function(A) {
		return this.className.match(new RegExp("(\\s|^)nicEdit-" + A
				+ "(\\s|$)"));
	},
	addClass : function(A) {
		if (!this.hasClass(A)) {
	...
	getStyle : function(A, C) {
		...
		if (this.nodeType == 1) {
			return (B && B.getComputedStyle) ? B.getComputedStyle(this, null)
					.getPropertyValue(A) : this.currentStyle[bkLib.camelize(A)];
		}
	},
	remove : function() {
...
var bkLib = {
	...
	addEvent : function(C, B, A) {
		(C.addEventListener) ? C.addEventListener(B, A, false) : C.attachEvent(
				"on" + B, A);
	},
	toArray : function(C) {
	...
	noSelect : function(B) {
		if (B.setAttribute && B.nodeName.toLowerCase() != "input"
	...
	onDomLoaded : function(A) {
		...
		if (document.addEventListener) {
			document
					.addEventListener("DOMContentLoaded", bkLib.domLoaded, null);
		} else {
			if (bkLib.isMSIE) {
				document
						.write("<style>.nicEdit-main p { margin: 0; }</style><script id=__ie_onload defer "
								+ ((location.protocol == "https:") ? "src='javascript:void(0)'"
										: "src=//0") + "><\/script>");
...
var nicEditor = bkClass.extend({
	construct : function(C) {
		...
		bkLib.addEvent(document.body, "mousedown", this.selectCheck
		...
		.extend({
			...
			construct : function(G, D, C) {
				...
				if (H || this.options.hasPanel) {
					...
					E[(B) ? "height" : "maxHeight"] = (this.ne.options.maxHeight) ? this.ne.options.maxHeight
							+ "px"
							: null;
					this.editorContain = new bkElement("DIV").setStyle(E)
							.appendBefore(G);
					var A = new bkElement("DIV").setStyle({
						width : (newX - 8) + "px",
						margin : "4px",
					...
					if (H) {
						...
						if (F) {
							bkLib.addEvent(F, "submit", this.saveContent
									.closure(this));
						}
					}
					A.setStyle((B) ? {
			...
			init : function() {
				...
				this.elm.addEvent("mousedown",
						this.selected.closureListener(this)).addEvent(
						"keypress", this.keyDown.closureListener(this))
						.addEvent("focus", this.selected.closure(this))
						.addEvent("blur", this.blur.closure(this)).addEvent(
								"keyup", this.selected.closure(this));
			...
			selElm : function() {
				...
				if (C.startContainer) {
						...
						for ( var B = 0; B < D.childNodes.length; B++) {
							...
							if (C
									.compareBoundaryPoints(
											Range.START_TO_START, A) != 1
									&& C.compareBoundaryPoints(
											Range.END_TO_END, A) != -1) {
					...
					return $BK((this.getSel().type == "Control") ? C.item(0)
							: C.parentElement());
				}
			},
			saveRng : function() {
			...
			saveContent : function() {
				if (this.copyElm || this.options.hasPanel) {
					...
					(this.copyElm) ? this.copyElm.value = this.getContent()
...
var nicEditorIFrameInstance = nicEditorInstance
		.extend({
			savedStyles : [],
			init : function() {
				...
				for (itm in A) {
					this.savedStyles[bkLib.camelize(itm)] = this.elm
							.getStyle(itm);
				}
			...
			initFrame : function() {
				...
				this.frameDoc.addEvent("mousedown",
						...
						this.heightUpdate.closureListener(this)).addEvent(
						"keydown", this.keyDown.closureListener(this))
			...
			heightUpdate : function() {
				this.elmFrame.style.height = Math.max(
						this.frameContent.offsetHeight, this.initialHeight)
						+ "px";
			},
			nicCommand : function(B, A) {
...
var nicEditorPanel = bkClass.extend({
	...
	addButton : function(buttonName, options, noOrder) {
		...
		var type = (button.type) ? eval("(typeof(" + button.type
				+ ') == "undefined") ? null : ' + button.type + ";")
				: nicEditorButton;
		var hasButton = bkLib.inArray(this.buttonList, buttonName);
		if (type && (hasButton || this.ne.options.fullPanel)) {
			this.panelButtons.push(new type(this.panelElm, buttonName, options,
					this.ne));
			if (!hasButton) {
		...
		.extend({
			construct : function(D, A, C, B) {
				...
				this.button = new bkElement("DIV").setStyle({
					...
					zoom : 1,
					cursor : "pointer"
				}).addClass("button").setStyle(this.ne.getIcon(A, C)).appendTo(
						this.border);
				...
				B.addEvent("selected", this.enable.closure(this)).addEvent(
						"blur", this.disable.closure(this)).addEvent("key",
						this.key.closure(this));
				this.disable();
				this.init();
			},
			init : function() {
			...
			checkNodes : function(A) {
				...
				do {
					if (this.options.tags
							&& bkLib.inArray(this.options.tags, B.nodeName)) {
						this.activate();
						return true;
					}
				...
				if (this.options.css) {
					for (itm in this.options.css) {
						if (B.getStyle(itm,
								this.ne.selectedInstance.instanceDoc) == this.options.css[itm]) {
							this.activate();
							return true;
						}
					}
			...
			key : function(A, B) {
				if (this.options.key
						&& B.ctrlKey
						&& String.fromCharCode(B.keyCode || B.charCode)
								.toLowerCase() == this.options.key) {
					this.mouseClick();
					if (B.preventDefault) {
						B.preventDefault();
					}
...
var nicEditorPane = bkClass.extend({
	construct : function(D, C, B, A) {
		...
		if (A && !A.options.noClose) {
			this.close = new bkElement("div").setStyle({
				...
				width : "16px",
				cursor : "pointer"
			}).setStyle(this.ne.getIcon("close", nicPaneOptions)).addEvent(
					"mousedown", A.removePane.closure(this))
					.appendTo(this.pane);
		}
	...
	position : function() {
		if (this.ne.nicPanel) {
			...
			var A = B.pos();
			var C = A[0] + parseInt(B.getStyle("width"))
...
var nicEditorAdvancedButton = nicEditorButton.extend({
	init : function() {
		this.ne.addEvent("selected", this.removePane.closure(this)).addEvent(
				"blur", this.removePane.closure(this));
	},
	mouseClick : function() {
		if (!this.isDisabled) {
	...
	addForm : function(C, G) {
		this.form = new bkElement("form").addEvent("submit", this.submit
				.closureListener(this));
...
var nicButtonTips = bkClass.extend({
	construct : function(A) {
		this.ne = A;
		A.addEvent("buttonOver", this.show.closure(this)).addEvent("buttonOut",
		...
		.extend({
			construct : function(D, A, C, B) {
				...
				this.control = new bkElement("div").setStyle({
					...
					height : "18px",
					width : "16px"
				}).addClass("selectControl").setStyle(
						this.ne.getIcon("arrow", C)).appendTo(this.items);
...
var nicEditorFontFormatSelect = nicEditorSelect.extend({
	...
	init : function() {
		...
		for (itm in this.sel) {
			...
			this.add("<" + A + ">", "<" + itm
					+ ' style="padding: 0px; margin: 0px;">' + this.sel[itm]
					+ "</" + A + ">");
		}
	}
});
...
var nicLinkButton = nicEditorAdvancedButton
		.extend({
			addPane : function() {
...
var nicUploadButton = nicEditorAdvancedButton
		.extend({
			nicURI : "/navigation/servlet/fileUpload.servlet",
			errorText : "Failed to upload image",
			addPane : function() {
				if (typeof window.FormData === "undefined") {
					return this
							.onError("Image uploads are not supported in this browser, use Chrome, Firefox, or Safari instead.");
				}
				this.im = this.ne.selectedInstance.selElm().parentTag("IMG");
				var A = new bkElement("div").setStyle({
					padding : "10px"
			...
			onUploaded : function(B) {
				...
				if (this.im) {
					this.im.setAttributes({
						src : D,
						width : (A && B.image.width) ? Math.min(A,
								B.image.width) : ""
					});
				}
			}
