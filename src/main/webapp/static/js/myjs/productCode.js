 ...
 */	
	...
	function saveD(){
		...
		var fields = dname + ',halfwanybus,' + dtype + ',halfwanybus,' + dbz + ',halfwanybus,' + isQian + ',halfwanybus,' + ddefault + ',halfwanybus,' + flength;;
	...
	function editField(value,msgIndex){
		$("#dialog-add").css("display","block");
		var efieldarray = value.split(',halfwanybus,');
	...
	function appendC(value){
		var fieldarray = value.split(',halfwanybus,');
		$("#fields").append(
			'<tr>'+
	...
	function isSame(value){
		for(var i=0;i<arField.length;i++){
			var array0 = arField[i].split(',halfwanybus,')[0];
			if(array0 == value){
				return false;
			}
		}
